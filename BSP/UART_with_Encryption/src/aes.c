...
static uint8_t cipher_text[16];
static uint8_t decrypted_text[16];
...
uint8_t* aes_decryption(uint8_t* encrypted_text)
{
    ...
    add_round_key(decrypted_text, 10);

    for (int i=9; i>0; i--)
    {
        inv_sub_bytes(decrypted_text, 16);
        inv_shift_rows(decrypted_text);
        add_round_key(decrypted_text, i);
        inv_mix_columns(decrypted_text);
    }

    inv_shift_rows(decrypted_text);
    inv_sub_bytes(decrypted_text, 16);
    add_round_key(decrypted_text, 0);
