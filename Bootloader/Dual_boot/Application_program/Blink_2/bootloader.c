...
void bootloader_Reset_Handler()
{
    ...
    GPIOA->MODER |= GPIO_MODER_MODE9_1 | GPIO_MODER_MODE10_1; // Set PA9, PA10 to Alternate Function mode
    GPIOA->AFR[1] |= (7 << 4) | (7 << 8); // Set AF7 (USART1)
    ...
    while(GPIOA->IDR & 1)
    {
        ...
        if (timeout >= 100000) // 3-4 sec delay
        {
            flag = 0;
            break;
        }
