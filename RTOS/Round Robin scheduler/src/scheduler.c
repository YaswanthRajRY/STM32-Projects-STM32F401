...
{	
	...
	executing_task = executing_task->next;						// move next task

	while (executing_task->tcb.task_state != READY)				// move to next task if scheduled task is blocked
	{
	    executing_task = executing_task->next;
	}
...
void SysTick_Handler(void) 
{
	...
	update_task_delay();										// set PendSV to trigger interrupt
    set_PendSV();
}
